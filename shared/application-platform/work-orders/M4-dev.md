...
- **OPEN DECISION — government-fee collection (confirm with owner before building totals):** does Tech Rig **charge** government fees (collect + remit to FMCSA/state) or only its **service fees** (customer pays gov fees directly)? `services.md` shows gov fees "separately"; the **$1,350 package explicitly includes** the MC + UCR-0-2 gov fees (so those ARE collected). Default assumption to confirm: **collect gov fees where the registry includes/states them; show them as a separate line; do not blend into the service fee.** Flag this to the orchestrator; do not guess silently for à-la-carte gov fees.
