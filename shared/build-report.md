...
### 11.13 BOC-3 + UCR combo pricing, DQ files bundle-rate discount, 2026-08

Owner-directed, planned and approved before implementation (plan mode). Fixes a real gap between the email outreach campaign, which advertises BOC-3 + UCR together for $200 to carriers in the UCR 0-2 power-unit bracket, and the live quick-buy site, which was charging $100 + $80 + $46 = **$226** for the same pair. Quick-buy only (`/apply` and the four fixed bundles are untouched).

**`computeQuickBuyPricing()` (`dev/lib/services-registry.ts`)** now layers two additional adjustments on top of its existing UCR-gov-fee-folding behavior:
1. **BOC-3 + UCR combo**: when both are selected and the resolved UCR bracket is exactly `"0-2"`, the pair totals a flat **$200** — BOC-3 keeps its normal $100, and the discount lands on the UCR line (new derived constants `BOC3_UCR_COMBO_TOTAL` = 200, `BOC3_UCR_COMBO_UCR_PORTION` = `200 - SERVICES["boc-3"].standalonePrice`, so the split stays correct if BOC-3's price ever changes). Every other bracket keeps today's separate itemized pricing, unchanged.
2. **DQ files bundle-rate discount**: whenever DQ files is selected alongside BOC-3 and/or UCR (independent of #1, independent of UCR's bracket, and even if UCR isn't selected at all), DQ reprices via the existing `DQ_BUNDLE_TABLE` (1 driver $200, 2 drivers $350, 3 drivers $450) instead of `DQ_STANDALONE_TABLE` — reusing already-reviewed pricing rather than introducing a new flat-per-driver tier (a real numeric difference for 2-3 drivers was flagged and confirmed with the owner before implementing).

The function's old early-return (`if (!selected.includes("ucr")) return computePricing(...)`) was removed since it would have skipped adjustment #2 for a BOC-3 + DQ order with no UCR; both adjustments are now evaluated independently against the same `computePricing()` base.

Each overridden line carries an honest `note` (already rendered under the price on the pay page and in the receipt email, so no new UI needed) — e.g. "BOC-3 + UCR combo price for the 0-2 bracket, $26 off the standard combined price," with the $26 derived from the actual figures, never hardcoded.

**`dev/app/buy/order/[orderId]/review/review-form.tsx`**: the "You may also need" checklist's price labels changed from static per-service prices to real, discount-aware ones, computed against the *current* selection (`computeQuickBuyPricing([...allSelected, key], ...).total - pricing.total` for BOC-3/UCR-style flat options; the 1-driver anchor from whichever DQ table currently applies for DQ files, since driver count isn't chosen until it's checked) — so a buyer sees the savings on the checkbox row itself before checking it, not just in the total afterward. This was an explicit, confirmed design decision (asked and approved before implementing), not an assumption.

No changes needed anywhere else — the pay page, `/api/quick-buy-checkout`'s charge amount and `filings` sync, the receipt email, and the admin "paid" alert all consume `computeQuickBuyPricing()`'s output generically and reflect the new numbers automatically, same reuse path already verified when the stale-UCR-price bug was fixed.

**Verified two ways:**
- A standalone `tsx` assertion script (no DB/Next.js runtime needed, `lib/services-registry.ts` has zero external imports) covering 8 cases: BOC-3+UCR at 0-2 → $200; BOC-3+UCR at 3-5 (5 units) → $318 unchanged; BOC-3+DQ (1 driver, no UCR) → $300; UCR+DQ (2 drivers, no BOC-3) at 0-2 → $476 (UCR itself isn't discounted without BOC-3); BOC-3+UCR+DQ (1 driver) at 0-2 → $400; BOC-3 alone → $100; DQ alone unchanged at both 1 and 2 drivers. All 8 passed.
- Live click-through on the dev server against a real 0-2-bracket carrier (USDOT 4581955, 1 power unit): confirmed the UCR upsell row shows $100 (not $126) and DQ files shows $200/driver (not $250) before checking either box; checking UCR alone brought the total to exactly $200 with the combo note visible; adding DQ files (1 driver) brought it to exactly $400 with its own discount note visible. Stopped short of completing an actual test payment (not needed to verify the pricing logic itself).

`npx tsc --noEmit`, `eslint`, and `next build` all pass clean.

**Bug found and fixed same day: the upsell checklist's price only matched the eventual breakdown in one direction.** The owner reported it live: on a UCR-primary order, the checklist showed BOC-3 at $76 (not $100), and after checking it the breakdown showed BOC-3 $100 and UCR $100, numbers that didn't visually reconcile with what was previewed. Root cause: `upsellPriceLabel()` in `review-form.tsx` was showing the **net total delta** of adding a candidate (`computeQuickBuyPricing([...allSelected, key]).total - pricing.total`), not that candidate's own resulting line price. Since the BOC-3+UCR combo always keeps BOC-3's line flat at $100 and moves the entire $26 discount onto the UCR line, adding BOC-3 to an existing UCR order *also* retroactively drops UCR's already-visible $126 line to $100 — the true net delta really is $74 ($100 new BOC-3 line minus $26 off the existing UCR line), but that $74 doesn't correspond to any single line the visitor ever sees, so the checklist number and the breakdown looked inconsistent. It happened to look correct in the opposite direction (BOC-3 primary, UCR as the upsell) purely by coincidence: BOC-3's own price never changes when UCR is added, so the net delta and UCR's own final price were numerically identical there, masking the bug. **Fixed** by changing `upsellPriceLabel()` to look up the candidate's own line in the hypothetical combined pricing (`withCandidate.lines.find((l) => l.key === key)`) instead of computing a net delta — the checklist number now always equals what that same service's row will read in the breakdown once checked, and any OTHER line's change (e.g. UCR dropping from $126 to $100) stays visible and self-explained by its own `note` in that same breakdown, rather than being smeared into the candidate's preview number. Re-verified live in both directions (BOC-3 primary → UCR upsell, and UCR primary → BOC-3 upsell): both now show $100 on the checklist and $100 + $100 = $200 in the breakdown, consistently.

`npx tsc --noEmit`, `eslint`, and `next build` all pass clean.

**Follow-up same day: added an explicit "Save $X" line to the upsell checklist** (owner request, to promote the combo/discount rather than leave the visitor to notice the lower price on their own). `upsellPriceLabel()` was folded into a new `upsellInfo()` in `review-form.tsx` that returns both the price label and a `savings` figure, computed generically for every upsell key with no per-service special-casing: `pricing.total + soloTotal(key) - withCandidate.total`, where `soloTotal` is what the candidate would cost entirely on its own (via `computeQuickBuyPricing([key], ...)`, so it's automatically $126 for UCR alone, not a naive $80, since quick-buy's own gov-fee folding already applies at that level). This single formula correctly produces $26 for BOC-3/UCR either direction, $50 for DQ files, and exactly $0 (so nothing renders) for Clearinghouse/Consortium, without the component needing to know which combo rule is in play. Rendered as a green (`text-status-active`) line under the existing reason line, only when `savings > 0`: "Save $26 by adding this to your order."

Verified live (USDOT 4581956, UCR primary, 2 power units): BOC-3 row shows "$100 / Save $26 by adding this to your order," DQ files shows "$200 per driver / Save $50 by adding this to your order," and Clearinghouse/Consortium show no savings line at all, correctly.

`npx tsc --noEmit`, `eslint`, and `next build` all pass clean.

### 11.14 Consortium + Clearinghouse multi-service discount; upsell styling refinements, 2026-08

Owner-directed. Consortium and Clearinghouse each now drop to a flat quick-buy price whenever bought alongside **any** other quick-buy service (including each other) — not gated to a specific pair like the BOC-3+UCR combo. New constants in `dev/lib/services-registry.ts`, following the same pattern as the existing adjustments: `CONSORTIUM_MULTI_SERVICE_PRICE = 120` (standard $175), `CLEARINGHOUSE_MULTI_SERVICE_PRICE = 100` (standard $125). Deliberately separate from `SERVICES[...].bundlePrice` (the /apply four-fixed-bundle price: $150 for Consortium, $100 for Clearinghouse) — Clearinghouse's numbers happen to coincide today, Consortium's doesn't, and reusing the bundle price would make a future `/apply`-bundle price change silently move this too. `computeQuickBuyPricing()`'s early-return guard and adjustment list were extended (`hasConsortium && multiService`, `hasClearinghouse && multiService`, where `multiService = new Set(selected).size > 1`) alongside the existing UCR/DQ logic, same `.map()`-over-`lines`/`filings` pattern.

No changes needed in `review-form.tsx`: the checklist's `upsellInfo()` and the payment summary's `normalLinePrice()` (§11.11-13) are fully generic over `computeQuickBuyPricing()`'s output, so both correctly picked up the new discount, "was" price, and "Save $X" line with zero UI code changes — confirmed live rather than assumed.

**Verification**: extended the standalone assertion script to 14 cases (was 8), covering Consortium alone ($175, unchanged), Clearinghouse alone ($125, unchanged), each paired with BOC-3, the two paired with **each other** (proving "any other service" includes them counting for one another), and a full 5-service stack (BOC-3 $100 + UCR $100 + DQ $200 + Consortium $120 + Clearinghouse $100 = $620). All 14 passed. Live click-through (USDOT 4581943, BOC-3 primary, all 4 upsells added) reproduced the identical $620 total and each individual struck-through/discounted price.

**Follow-up refinements from live owner feedback during this same pass**, all in `review-form.tsx`:
- DQ files' struck-through "was" price no longer carries the "per driver" suffix (reads plain "$250"), while the current price keeps it ("$200 per driver") — the suffix only ever applied to the live/current figure.
- Restyled the strikethrough treatment in both the checklist and the payment summary: the "was" price is now a smaller (11px), lighter (`text-slate/60`) line stacked *above* the current price, instead of an inline same-size/weight strikethrough before it.
- **The payment summary's per-line description no longer switches between a dynamic discount-explanation note and a generic fallback note.** It now always shows the exact same static "why you need this" line used in the upsell checklist (`QUICK_BUY_UPSELL_REASON`), so the description text is identical and constant between the two surfaces regardless of whether that line happens to be discounted — the struck-through price already communicates the discount, so the text no longer needs to.
