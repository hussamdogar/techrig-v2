...
8. Closing CTA band (full-bleed Ink, the template's closing emphasis): the brief's closing line "Need a BOC-3 to activate your authority? File it directly with an FMCSA-listed process agent." as a styled paragraph, then the single dominant button "File my BOC-3" (Signal amber, Ink text) routing to `/buy/boc-3/`. One action only.

9. Mega-footer (global): the complete filing list lives here.

## New flow: /buy/boc-3/ (USDOT confirm-and-pay)
Three screens, `noindex`, no header/footer chrome beyond a minimal wordmark bar (this is a checkout flow, not a content page; keep it fast and free of navigation that invites bail-out).
- **Entry** (`/buy/boc-3/`): single centered card, mono label "Start your BOC-3 filing," one USDOT input (numeric, large touch target), primary amber "Look up my USDOT" button. No other fields, no account prompt.
- **Confirm** (`/buy/boc-3/[usdot]/`): a curated carrier-identity card (legal name, DBA, USDOT, MC number, address, power units) in the same Row/DocketSection line style as the existing `/lookup/[usdot]/` page (reuse, do not redesign). Two editable fields beneath it, pre-filled from the pulled record: email and phone, each with a small "pulled from your FMCSA record, edit if needed" caption in Slate. No extra numeric field for BOC-3. Price chip: flat "$100" from the single source. Primary amber button "Confirm and pay." A quiet "This isn't my carrier" link resets to entry.
- **Pay** (`/buy/[orderId]/pay/`): Stripe Payment Element in the standard card treatment, price chip repeated, no other fields.
- **Thank-you** (`/buy/[orderId]/thank-you/`): confirmation checkmark motif (line icon, not a photo), reference id in mono, one paragraph on what happens next (we file your BOC-3, it posts to your FMCSA record), no dashboard link or login prompt since no account exists. Optional quiet link back to `/` or the compliance hub.
This flow shares the design system's tokens, type, and line-icon language with the rest of the site but is deliberately stripped of marketing chrome: single column, one action per screen, nothing that competes with "confirm" or "pay."
...
## CRO treatment
- One dominant action, "File my BOC-3", Signal amber, repeated at hero and close, both routing to `/buy/boc-3/`; the mid-page CTA is a subordinate Steel text link to the same route.
