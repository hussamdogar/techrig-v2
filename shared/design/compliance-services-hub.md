...
3. Featured offer: "The full compliance package" (the recommended path, highest visual weight after hero)
   ...
   - The price as a flat price chip in the mono treatment: $1,350 (single source). One-line value paragraph.
   - Package contents as a two-column checklist, each item an inline 1 to 3 word contextual link with a small line icon (USDOT, MC authority, BOC-3, UCR, DQ files, Clearinghouse, consortium, pre-employment drug test, IRP, IFTA; LLC, insurance, and ELD are NOT in the package, they are a la carte). The package price already includes the MC and UCR (0 to 2 bracket) government fees; IRP and IFTA government fees are the exception, calculated from mileage and states. Check marks use the status-active color, never color alone (icon + label).
...
4. H2 "What our DOT compliance services cover": the service-card grid (this is the canonical home of the service-card component; design once, reused on every hub). 3-column responsive, equal-height, Cloud surface, 1px outline. Each card = small line icon + service name (Steel link) + one plain-language line + price chip from the single source. Cards (14): USDOT ($300 standalone), MC authority ($600, includes the USDOT number), BOC-3 ($100), UCR ("from $100" + "+ gov fee by fleet size"), IRP plates ($175 setup + state fees), IFTA ($175 setup + state fees), Clearinghouse Registration ($100), drug and alcohol consortium ($150), DQ files ($200 per driver), ELD setup ("Contact for quote"), MCS-150 updates ($125), trucking LLC ("Contact for quote", pricing on hold), MC reinstatement/deactivation (from $200), USDOT reactivation/deactivation ($125). These are content-bearing cards (real price, real service), not the decorative icon-triplet reflex.
...
Eye path: H1 -> primary amber CTA -> the $1,350 featured package (the path we want most carriers to take) -> the à la carte card grid for those who want one filing -> proof -> process -> FAQ -> single closing CTA. Signal amber is rationed to the primary action (hero, package panel, close); all card links and the secondary anchor are Steel and subordinate. One primary action per view.
...
## CRO treatment
- The $1,350 package is the friction-removing hero offer: "do not assemble this piece by piece." It gets the featured treatment and the primary CTA, so the low-effort path is the obvious one.
