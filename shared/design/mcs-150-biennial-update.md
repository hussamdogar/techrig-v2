...
2. Hero (Paper, asymmetric two-column; copy-first stack on mobile)
   ...
   - Right (signature visual): the Authority Status Tracker, scoped to where this filing sits. This is upkeep on an already-active authority, so the tracker shows "Authority active" as the maintained state with the MCS-150 update as the keep-active action, not a fresh application sequence. It honestly frames the page as protecting an active number rather than activating a new one. No guaranteed dates; the deactivation risk is shown as a state, not a countdown.
...
Eye path: H1 -> primary amber CTA -> the tracker (active-number-maintained framing) -> what it is -> when you need it (triggers + lapse risk) -> what we do + the $125 chip -> the worked-example proof -> the MOTUS/PIN expertise -> FAQ -> dispatch handoff -> single closing CTA. Signal amber is rationed to the primary action (hero, close, header CTA). The mid-page CTA, all internal links, and the handoff are Steel and subordinate. One primary action per view.

## Imagery and illustration
No photography. Hero: the Authority Status Tracker in its active-maintained state. One small line icon per trigger in the "when you need it" list (a moving/address icon, a fleet/truck-count icon, an operation-change icon, a reactivate/refresh icon, a clock for biennial). The MOTUS section may use one small line process diagram (blocked path -> callback -> unlocked). Worked example uses the Steel-rule callout, no illustration. All single-line SVG, lightweight for CWV.

## Motion
Minimal. Hero tracker staggered reveal (<=200ms steps). Optional one-time draw on the MOTUS process diagram when it enters view, no looping and not on every section. Accordion chevron rotates. Price chip static, no count-up on the "24 hours" figure (it is prose, not a live counter). Sticky mobile CTA appears after the hero. `prefers-reduced-motion` gives final static states.
