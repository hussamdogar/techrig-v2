...
- **M1 — R1+R2 LANDED + DB gate PASSED; stays ACTIVE pending 3 deploy-time checks.**
  ...
  - **M1 = BUILD-COMPLETE.** Deploy policy (owner, 2026-06-25): nothing deploys, even to preview, until the whole site is complete + QA'd. So M1's 3 deploy-time checks (QCMobile real-IP failover, Vercel KV, Lighthouse) moved to the Consolidated pre-launch QA ledger (`application-platform/03-roadmap.md`) and do NOT block M2. M1 flips to fully DONE when that ledger clears at launch.
