...
- **M6 ACTIVE (Dev-led)** — work order `work-orders/M6-dev.md`: Resend email lifecycle (welcome/payment receipt/24h+72h reminders w/ coupon/final-per-service/status-change), idempotent via `*_sent_at` guards, cron (`CRON_SECRET`), PDF acknowledgement+answers (`pdf-lib`) → Supabase Storage `documents` (`0006`). Verify logic locally (Resend sandbox); real deliverability + cron scheduling → QA ledger. CAN-SPAM on the promotional 72h email; honor ELD/insurance reframe in copy.
- **Platform progress: M0–M5 + M3-R1 all build-complete** (full revenue + tracking path working in test mode; 5 prod migrations). M6 active; M7 (subdomain 301 migration + hardening + final `/security-review`) remains. See `application-platform/03-roadmap.md` status ledger.
