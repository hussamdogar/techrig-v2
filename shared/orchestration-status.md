...
- **M3 ACTIVE (Dev-led)** — work order `application-platform/work-orders/M3-dev.md`: the unified service-driven application engine (service registry from `services.md`, `applications`+`filings` migration `0003`, dynamic stepper, carrier diff/MCS-150, autosave/resume, server-side pricing display, one filing per service). Unblocked by M2. ELD/insurance stay non-billable. **Recommend M1 R3 lands first or in parallel** — M3's OA-aware service pre-selection (`§6`) reads the enriched snapshot R3 produces.
