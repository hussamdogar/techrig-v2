...
- **M1 — R1+R2 LANDED + DB gate PASSED; stays ACTIVE pending 3 deploy-time checks.**
  ...
  - **OPEN — confirm with owner (orchestrator flag):** migration was applied to Supabase project `pqbynaaihauifomfhcxo` **"BOC-3 Test Project"** (the only project the provided token owns). Need to confirm this is the intended production target for the new platform. If it is a TEST project, then before launch: apply `0001` to the real production project and repoint `dev/.env.local` + Vercel env. This is the one item that could mean "M1 ran against test, not prod."
  - **Deferred to the Vercel preview (owner-agreed; orchestrator verifies then flips M1 → DONE):** (1) QCMobile backup failover on a real IP (sandbox got 403 from `mobile.fmcsa.dot.gov` — likely IP WAF; confirm webKey activated); (2) KV counter + rate-limit on real Upstash (sandbox unreachable; code falls back); (3) Lighthouse perf/CLS on `/`.
