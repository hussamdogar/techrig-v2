...
- **M7 ACTIVE (Dev-led, FINAL milestone)** — work order `work-orders/M7-dev.md`: subdomain 301s (`form.`/`boc-3.techrig.org` → `/apply/`), legacy drain-vs-ETL decision, **full platform `/security-review` (mandatory gate)**, Sentry, rate-limit/load sanity, and `shared/build-report.md` (the contract deliverable, satisfies Workstream A L3). **This is the convergence point: when M7 is build-complete, re-engage Workstream A (L1 crawl-union, L2 blog sign-off, L4) for ONE joint launch deploy that runs the consolidated QA ledger.**
- **Platform progress: M0–M6 + M3-R1 all build-complete** (full revenue + tracking + comms path in test mode; 6 prod migrations `0001–0006`). M7 (final) active.
