...
- **M4 ACTIVE (Dev-led)** — work order `work-orders/M4-dev.md`. Stripe (TEST mode) payment capture, server-priced from the registry, idempotent signature-verified webhook → `payments.paid`/`applications.paid`/`filings.queued`, migration `0004`. First money-touching milestone: **`/security-review` mandatory before build-complete.** **OPEN owner decision before charge totals: does Tech Rig collect government fees, or only service fees?** (The $1,350 package includes MC+UCR-0-2 gov, so those are collected; à-la-carte gov-fee collection needs confirmation.)
