...
- **M1 CODE-COMPLETE (committed `cf57ff8`), gate not yet fully passed → stays ACTIVE.** Shipped: `lib/lookup` dual-provider failover, `/api/lookup-usdot`, `leads`+`carrier_snapshots` migration `0001` (written, NOT applied), hero card (all states, noindex). Proven live in-sandbox against MOTUS (USDOT 3214567 → correct data; not_found/invalid paths correct; noindex header; home stays prerendered). Owner revisions (2026-06-25), land BEFORE the gate: (R1) Search → dedicated noindex results page `/lookup/[usdot]/` (not inline expand); card becomes an entry form; extract a shared `performLookup()`. (R2) Render the COMPLETE matrix docket on that page, not the 8-field card summary (two-step fetch already correct; gap was display scope). Then the gate (live env now in `dev/.env.local`, prod migration AUTHORIZED — pre-flight then apply): (a) apply `0001` after the read-only existence check; (b) Vercel preview with env; (c) verify backup provider (QCMobile webKey), Lighthouse perf/CLS, RLS. Dev session executes; orchestrator verifies evidence and flips M1 → DONE.
