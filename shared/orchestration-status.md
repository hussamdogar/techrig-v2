...
- **M5 ACTIVE (Dev-led)** — work order `work-orders/M5-dev.md`: progress tracking + filing lifecycle + back-office. `filing_events` table (`0005`) + admin/role model (`profiles.role`, first admin seeded manually) + admin-guarded status-transition API (state machine queued→filed→active→completed) + the `(admin)` board + the client progress timeline (reuses `AuthorityStatusTracker`) + surfacing `needs_mcs150`/diff. Admin authz is a privilege boundary — server-side checks only; flagged for the M7 security review. Build-complete on local + prod-DB verification (no preview).
- **Platform progress: M0–M4 + M3-R1 all build-complete** (full revenue path working in test mode). M5 active; M6 (email/PDFs) + M7 (subdomain 301 migration + hardening + final `/security-review`) remain. See `application-platform/03-roadmap.md` status ledger.
