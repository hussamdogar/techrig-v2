...
**Price line:** Tech Rig charges a service fee to file your IRP. [VERIFY exact IRP service fee with the client before publishing.] The state registration fees themselves are set by the jurisdictions and your mileage, and are paid separately. We show you which is our fee and which is the government fee before you pay.
...
## Schema (JSON-LD)
- `Service` (serviceType "IRP registration", provider {@id #org}, areaServed US, offers price [VERIFY service fee] USD).
- `BreadcrumbList`: Home > Compliance Services > IRP Registration.
- `FAQPage`.

## Proof / claims discipline
- IRP service fee marked `[VERIFY]`; never encode state fees as the price.
