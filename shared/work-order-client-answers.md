...
- **Dev D1-D9 + D13/S2: ✅ DONE** — committed to main (`0ef41fa`), orchestrator-verified (19 dev/-lane files; `full-package` = 9 items at $1,700; DQ tiers via `calculateDqFiles`; new `motus-migration` service; tracker grep-confirmed gone from both built pages). `next build` exits 0 (96 routes); **parity gate PASS**. L10 fully closed (DZ2 + D13). 3 new indexable pages (`/usdot-correction/`, `/ifta-quarterly-filing/`, `/motus-migration/`) → fold into the L1 crawl-union.
- **Dev D10/D11/D12: STILL OPEN** (owner-action / fast-follow, not in `0ef41fa`): D12 live Stripe `sk_/pk_/whsec_` (owner provides); D11 legacy import (needs the boc-3 DB export); D10 renewal reminders (post-launch fast-follow). None block launch build-readiness.
...
- Give each compliance page a service-specific CTA (primary + any "file my X" secondary), matching the new pages. Mapping: `ucr-registration`→`ucr`; `dot-registration`→`usdot`; `mc-registration`→`mc-authority`; `mc-dot-registration`→`mc-authority`; `fmcsa-clearinghouse-registration`→`clearinghouse`; `drug-and-alcohol-consortium`→`consortium`; `driver-qualification-files`→`dq-files`; `irp-registration`→`irp`; `ifta-registration`→`ifta`; `mcs-150-biennial-update`→`mcs-150`; `boc-3-filing`→`boc-3`.
- **Hubs (clarified 2026-06-25, resolving the S9 ambiguity):** the **dispatch hub `/services/`** stays on `/contact-us/` (dispatch is not an `/apply` filing). The **compliance hub `/compliance-services/`** is a card directory whose cards route to the individual compliance service pages (which now → `/apply`); it has no direct "file" CTA, so **no `/apply` CTA on the hub itself** and **no brief change for `compliance-services-hub.md`**. (Earlier "`services` hub → `/apply` generic" was imprecise — disregard.)
